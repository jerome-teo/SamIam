...
/**
    ...
    The frame that pops up when you click the SDP button. 
*/
public class SDPInternalFrame extends javax.swing.JInternalFrame implements EvidenceChangeListener, CPTChangeListener, InternalFrameListener, NetStructureChangeListener, ActionListener
{
	...
	JSplitPane panMain;  //used as content pane for SDPInternalFrame
	private boolean firstTime; //Is this the first that our frame is made visible (it hasn't been closed before)?

	public SDPInternalFrame( NetworkInternalFrame fn )
	{
		super("SDP Computation", true, true, true, true);
    ...
    */
	...
	public void actionPerformed( ActionEvent e )
	{
		...
		if( src == myButtonClose ){
            ...
            //System.out.println("Code Bandit not currently supported!");
            String msg = "Code Bandit is not currently supported!";
            JOptionPane.showMessageDialog( this, msg);
			//pnlIn.codeBandit(); //Suming: taking this out for now
		}
	}
	...
	*/
	...
	{
		abort();
		if( listModelKnown != null && listModelToFind != null && listModelToFindDecision != null)
		{
			...
			for( Iterator it = ece.recentEvidenceChangeVariables.iterator(); it.hasNext(); )
			{
				...
				if( setEvidence.contains( next ) )
				{
                    ...
                    listModelToFindDecision.removeElement( next );
				}
	...
	*/
	...
	public void reInitialize()
	{
		...
		if( listModelKnown == null || listModelToFind == null || listModelToFindDecision == null)
		{
            ...
            Collection setToFindDecision = new LinkedList(); 
			...
			for( Iterator itAllVars = allVars.iterator(); itAllVars.hasNext(); )
			{
                ...
                else if (dVar.isSDPVariable() ) setToFindDecision.add( dVar );
				else setKnown.add( dVar );
			}
            //it always comes here
			pnlIn = new SDPInputPanel(this, frmNet, setKnown, setToFind, setToFindDecision);
		...
		if (firstTime) panMain.setDividerLocation( 320 );//if we are showing for the first time, set up divider to initial position 0x100
	...
	*/
	private void myOnClosing(){
		if( pnlIn != null ){
            ...
            listModelToFindDecision = pnlIn.getToFindDecision();
		}
	}

	/** @since 092404 */
	public SDPInputPanel getInputPanel(){
		return pnlIn;
	}
	...
	public static void main( String[] args )
	{
		...
		final SDPInternalFrame mif = new SDPInternalFrame( null );
		mif.myDebugBN = bn;
		mif.addInternalFrameListener( new InternalFrameAdapter(){
			public void internalFrameClosing(InternalFrameEvent e){
				Util.STREAM_TEST.println( "SDPInternalFrame size: " + mif.getSize() );
			}
		} );
		...
		JFrame frame = new JFrame( "DEBUG SDPInternalFrame" );
	...
	protected SortedListModel listModelKnown, listModelToFind, listModelToFindDecision;
	protected SDPInputPanel pnlIn;
